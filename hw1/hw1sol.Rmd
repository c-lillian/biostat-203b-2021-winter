...
I ran `chmod +x middle.sh` to make the file executable by the owner. The output prints 5 lines until line 20 from the middle of the text file, which in pride_and_prejudice.txt specifies lines for the author and the editor (lines 16-20). `"$1"` refers to the variable `filename`, `"$2"` refers to the variable `end_line` which represents the line number we want to end the output printing at, and `"$3"` refers to the variable `num_lines` which refers to the number of lines we want to print, and the lines printed includes the line number dictated by `end_line`. We need the first line of the shell script in order to execute the commands using the `sh` shell, since we are using bash shell in the code chunk above. The exclamation mark in line 1 is formatting related.
