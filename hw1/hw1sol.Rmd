...
#ASK TA
I ran `chmod +x middle.sh` to make the file executable by the owner. The output prints 5 lines until line 20 from the middle of the text file, which in pride_and_prejudice.txt specifies lines for the author and the editor (lines 16-20). We need the first line of the shell script in order to execute the commands using the `sh` shell, since we are using bash shell in the code chunk above.
...
#ASK TA: Should I have the chunk auto evaluate?
