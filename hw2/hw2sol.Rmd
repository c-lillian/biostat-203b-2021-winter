...
`anchor_age` is supposed to represent the age of the patient during the `anchor year` listed for that same patient. However, the values of `anchor_age` seem off because there is a significant peak of over 60,000 entries where the entered `anchor_age` for a patient was equal to 0. This is a sign that there might be anomalies in data collection or unclear coding of data from the hospital system's end (maybe the patient's age at admission was coded 0 for unknown or underage). 
...
There are over 327 million rows in `chartevents.csv.gz`.
```{r}
...
This did not work because the file is too large.
...
chartevents_filtered_path <- paste0("/usr/203b-data/mimic-iv-derived-data/",
...
# I ran this locally to download data to my own machine:
scp -r lchen121@server.ucla-biostat-203b.com:/usr/203b-data/mimic-iv-derived-data d/

# I decided to do my own filtering on my local machine using the shell command:
zcat < "/d/mimic-iv-0.4/mimic-iv-0.4/icu/chartevents.csv.gz" | 
  awk -F, '{if ($6 == 220045 || $6 == 220050 || $6 == 220052 || 
  $6 == 220179 || $6 == 220181 || $6 == 220210 || $6 == 223761 || $6 == 220277) 
  print $1, $2, $3, $4, $6, $8} {OFS = ","}' | gzip > q7chartevents.csv.gz

# To transfer my filtered chart events back to the teaching server:
# scp filename username@server.ucla-biostat-203b.com:fullpathtohwfolder
scp q7chartevents.csv.gz lchen121@server.ucla-biostat-203b.com:/home/lchen121/biostat-203b-2021-winter/hw2/
...
v_chartevents <- c(220045, 220052, 220181, 220050, 220179, 
            223761, 223761, 229862, 220210)

q7 <- chartevents_filtered_itemid %>% 
   filter(itemid %in% v_chartevents)
...
q8 <- icustays %>% 
  ...
  # deathtime missing for some hospital_expire_flag = 1, use dischargetime instead of deathtime
