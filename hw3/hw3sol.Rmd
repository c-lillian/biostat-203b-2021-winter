...
```{r}
# TODO
labevents_tble <- tbl(con, Id(schema = "mimic_hosp", table = "labevents")) %>% 
  print(width = Inf)
```
...
```{r}
# TODO
chartevents_tble <- tbl(con, Id(schema = "mimic_icu", table = "chartevents")) %>% 
  print(width = Inf)
```
...
icu_cohort <- icustays_tble %>%
  ...
  # create admitage variable
  mutate(admitage = year(admittime) - anchor_year + anchor_age) %>% 
  # keep only adults
  filter(admitage >= 18) %>% 
  # merge in labevents and chartevents tables
  # display the SQL query
  # collect SQL query result into memory as a tibble
  # create an indivator for 30-day mortality
  # save the final tibble to an `icu_cohort.rds` R data file in `mimiciv_shiny`
  print(width = Inf)
